...
class Restaurant(db.Model):
    ...
    id = db.Column(db.Integer, primary_key=True)
    name = db.Column(db.String(100), nullable=False) # A brief overview of the restaurant
    description = db.Column(db.Text)  # Detailed description of the restaurant
    contact_no = db.Column(db.String(15))  # Contact number
    email = db.Column(db.String(100), unique=True)  # Email address # Website URL
    ...
    zip_code = db.Column(db.String(20)) # Rating (0-5 scale)
    ...
    def __init__(self, name, description, contact_no, email, address, city, state,
                 zip_code, image, logo, owner_id):
        self.name = name
...
class Staff(db.Model):
    ...
    shift = db.Column(db.String(50))   # Days of the week the staff is scheduled to work
    ...
    def __init__(self, username, staffcode, gender, location, age, shift, role, image, restaurant_id):
...
class RestTable(db.Model):
    ...
    id = db.Column(db.Integer, primary_key=True)  # Unique identifier for each table record
    restaurant_id = db.Column(db.Integer, db.ForeignKey('restaurants.id'), nullable=False)  # Foreign key linking to the restaurant
    table_number = db.Column(db.Integer, nullable=False)  # Table number in the restaurant
    capacity = db.Column(db.Integer, nullable=False)  # Maximum number of people the table can accommodate
    status = db.Column(db.String(20), nullable=False, default="available")  # Status of the table (e.g., available, occupied)

    # Relationship to Orders
    orders = db.relationship('Order', backref='rest_table', lazy=True)  # One-to-many relationship with Orders
    
    restaurant = db.relationship('Restaurant', backref='tables')  # Relationship to Restaurant

    def __init__(self, restaurant_id, table_number, capacity, status='available'):
        self.restaurant_id = restaurant_id
    ...
    def __repr__(self):
        return f"<RestTable ID: {self.id}, Table Number: {self.table_number}, Status: {self.status}>"
...
class MenuItem(db.Model):
    ...
    buying_price = db.Column(db.Float, nullable=False)  # Price of the menu item
...
class Department(db.Model):
    ...
    dept_name = db.Column(db.String(100), nullable=False)  # Name of the department
    restaurant_id = db.Column(db.Integer, db.ForeignKey('restaurants.id'), nullable=False)  # Foreign key to the restaurant
    ...
    def __init__(self, dept_name, restaurant_id):
        self.dept_name = dept_name
        self.restaurant_id = restaurant_id

    def __repr__(self):
        return f"<Department ID: {self.id}, Name: {self.dept_name}>"
...
class Order(db.Model):
    ...
    table_id = db.Column(db.Integer, db.ForeignKey('rest_tables.id'), nullable=False)  # Foreign key referencing the table
    ...
    rest_table = db.relationship('RestTable', backref='orders')  # Relationship to RestTable
